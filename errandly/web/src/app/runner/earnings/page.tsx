    ...
    <div className="p-4 pb-20 space-y-5">
      ...
      <div className="bg-card rounded-2xl border border-border p-5 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-foreground">Payout bank account</h3>
        </div>
        <p className="text-xs text-muted-foreground">Required before withdrawals are processed.</p>
        <input
          value={bankName}
        ...
        {summaryCards.map((card) => (
          <div key={card.label} className="bg-card rounded-2xl border border-border p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-bold text-foreground text-base">₦{(card.value as number).toLocaleString()}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {showWithdraw && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end">
          <div className="bg-card rounded-t-3xl w-full p-6">
            <h3 className="font-bold text-xl text-foreground mb-2">Withdraw Earnings</h3>
            <p className="text-muted-foreground text-sm mb-4">Funds processed within 24 hours to your bank account.</p>

            <div className="grid grid-cols-3 gap-3 mb-4">
              {[2000, 5000, 10000, 20000, 30000, 50000].map((a) => (
                <button key={a} onClick={() => setAmount(a)} className={`py-3 rounded-xl font-semibold text-sm transition ${amount === a ? 'bg-[#FF6B00] text-white' : 'bg-gray-100 text-foreground'}`}>
                  ₦{a.toLocaleString()}
                </button>
              ))}
            </div>

            <input type="number" value={amount} onChange={(e) => setAmount(parseInt(e.target.value) || 0)} className="w-full px-4 py-3 border border-input rounded-xl focus:outline-none focus:ring-2 focus:ring-primary mb-4 text-lg font-bold" placeholder="Custom amount" />

            <div className="flex gap-3">
              <button onClick={() => setShowWithdraw(false)} className="flex-1 border border-input text-foreground font-semibold py-3 rounded-xl">Cancel</button>
              <button
