...
export default function ConversationThread({ backHref }: Props) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)] pb-20">
      <div className="flex items-center gap-3 p-4 border-b border-border bg-card sticky top-16 z-10">
        <Link href={backHref} className="p-2 rounded-lg hover:bg-muted">
              ...
              >
                <div
                  className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm ${
                    mine ? 'bg-[#FF6B00] text-white' : 'bg-card border border-border text-foreground'
                  }`}
                >
                  <p>{msg.content}</p>
      ...
      >
        <input
          ...
          className="flex-1 border border-input rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40"
        />
        <button
          type="submit"
